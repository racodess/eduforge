...
def render_generation_sidebar() -> None:
    ...
    with st.sidebar:
        with st.container(border=True):
            st.markdown("<h1 style='text-align:center;'>Generate</h21>", unsafe_allow_html=True)
            ...
            if st.button("", key="gen_button", type="secondary", icon=":material/send:", use_container_width=True):
                final_text = ""
...
def render_chatbot_sidebar() -> None:
    ...
    with st.container(border=True):
        st.markdown("<h1 style='text-align:center;'>Chat</h21>", unsafe_allow_html=True)
